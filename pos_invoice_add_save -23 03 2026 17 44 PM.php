...
foreach ($items as $item) {
    ...
    if ($costField) {
        ...
        // 🔒 OEM + NEW → cannot go below cost
        if ($type === 'OEM' || $type === 'NEW') {
            if ($price < $cost) {
                throw new Exception("ERROR: Cannot sell below cost price (Item: " . $name . ")");
            }
        }

        // 🔒 TP → price cannot be changed
        if ($type === 'TP') {

            $stmtTP = $conn->prepare("SELECT selling_price FROM third_party_parts WHERE id=?");
            $stmtTP->bind_param("i", $id);
            $stmtTP->execute();
            $resTP = $stmtTP->get_result();
            $tpRow = $resTP->fetch_assoc();

            $originalPrice = (float)$tpRow['selling_price'];

            if ($price != $originalPrice) {
                throw new Exception("ERROR: Third Party price cannot be changed");
            }
        }
    }
