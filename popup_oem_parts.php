...
th{background:#111;color:#4da6ff;}
...
<input type="text" id="searchBox" placeholder="Search OEM parts...">
...
<tr 
data-qty="<?= $qty ?>"
<?= $qty > 0 ? "onclick='selectOEM(".json_encode($row).")'" : "" ?>
style="<?= $qty > 0 ? "" : "opacity:0.3;" ?>"
>

<td><?= $row['id'] ?></td>

<td class="meta"><?= h($row['oem_number']) ?></td>
...
<td><?= h($row['supplier_oem_invoice_id']) ?></td>
